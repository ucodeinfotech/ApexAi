...
VN = list(VER.keys())
CB = [VER[v][0] for v in VN]
CR = [VER[v][1] for v in VN]
NV = len(VN)
CH_VALS = sorted(set(CB))
print("CH_VALS:", CH_VALS)
...
for sym in ["NIFTY50", "SENSEX"]:
    ...
    for i in range(60, len(h1)):
        ...
        # Pre-compute P&L for all CH values
        pnls = {}
        for cv in CH_VALS:
            he = ep
        ...
        if 45 not in pnls or pnls[45] is None:
            continue
...
# Evaluate all
print(f"\n{'='*95}")
print(f"{'VERSION':>40s} {'BASE Net':>12s} {'N':>5s} {'SKIP Net':>12s} {'N':>5s} {'DELTA':>10s} {'%':>7s}")
print(f"{'='*95}")
...
for v in range(NV):
    ...
    # Per-version training loss median
    train_losses = []
    for t in train:
        p = get_pnl(t, cb, cr)
    ...
    for i in range(len(base_p)):
        ...
        skip = False
        if i > 0 and not base_w[i-1] and base_p[i-1] < th:
            skip = True
    ...
    skip_net = sum(skip_p)
    skip_n = len(skip_p)
    delta = skip_net - base_net
    pct = (skip_net / base_net - 1) * 100 if base_net != 0 else 0
    results.append((skip_net, vn, base_net, skip_net, base_n, skip_n, delta, pct))
    print(f"{vn:>40s} Rs{base_net:>+9,.0f} {base_n:>5d} Rs{skip_net:>+9,.0f} {skip_n:>5d} Rs{delta:>+8,.0f} {pct:>+7.1f}%")
...
print(f"{'Rank':>4s} {'VERSION':>40s} {'Base Net':>12s} {'Skip Net':>12s} {'%':>7s}")
...
for rank in range(min(3, len(results))):
    sn_v, vn, bn_v, sn_v_total, bn_n, sn_n, delta_v, pct_v = results[rank]
    v = VN.index(vn)
    cb = CB[v]
    cr = CR[v]
    
    print(f"\n  [{vn}] (+{pct_v:+.1f}%)")
    print(f"  {'Year':>6s} {'Base':>10s} {'Skip':>10s} {'%':>7s} {'N':>5s}")
    ...
    for yr in sorted(set(t["year"] for t in test)):
        ...
        if len(yr_p) < 3:
            continue
        base_yr = sum(yr_p)
        wins = [x > 0 for x in yr_p]
        
        # Prior (pre-this-year training)
        ...
        th0 = np.median(prior_losses) if len(prior_losses) > 5 else -5000
        
        sp = []
        pl = list(prior_losses.copy())
        for i in range(len(yr_p)):
            pnl = yr_p[i]
            pr = [x for x in pl if x < 0]
            th = np.median(pr) if len(pr) > 5 else th0
            skip = False
            if i > 0 and not wins[i-1] and yr_p[i-1] < th:
                skip = True
            if not skip:
                sp.append(pnl)
                pl.append(pnl)
        ...
        print(f"  {yr:>6d} Rs{base_yr:>+8,.0f} Rs{skip_yr:>+8,.0f} {imp:>+6.1f}% {len(sp):>5d}")
    
    print(f"  TOTAL: Rs{bn_v:>+9,.0f} -> Rs{sn_v_total:>+9,.0f}")
