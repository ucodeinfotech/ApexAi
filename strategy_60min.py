...
# Penny filter (apply from daily avg price)
daily = con.execute("SELECT symbol, close FROM feature_store WHERE timeframe='1day' AND datetime >= '2024-06-24'").fetchdf()
penny_syms = set(daily.groupby('symbol')['close'].mean().pipe(lambda s: s[s < 50]).index)
